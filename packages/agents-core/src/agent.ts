...
import { mcpConfigsToServers } from './mcpConverters';
import {
  AgentType,
  parseCodexAgentType,
} from './extensions/codex';
...
> {
  name: string;
  /**
   * Identifies the execution environment backing this agent. Defaults to 'llm' for standard models,
   * or one of the Codex agent roles when delegating to the Codex CLI.
   */
  agentType: AgentType;
  /**
   * Runtime instance for executing this agent.
   *
   * REQUIRED for computer-use agents (agentType: 'computer').
   * Determines whether the agent executes locally (LocalRuntime) or in the cloud (CloudRuntime).
   *
   * NOT USED for LLM agents (agentType: 'llm') - they use the standard model provider system.
   */
  runtime?: Runtime;
  /**
   * Filesystem workspace the Codex agent should operate within. Defaults to the current working directory.
   */
  workspace?: string;
  /**
   * Optional session identifier to reuse context across runs.
   * For computer agents: Reuses Codex CLI session context.
   * For LLM agents: Can be used for conversation continuity.
   */
  sessionId?: string;
  /**
   * Reasoning effort hint ("none" | "low" | "medium" | "high").
   * For computer agents: Passed to Codex CLI.
   * For LLM agents: Can influence model behavior if supported.
   */
  reasoningEffort?: 'none' | 'low' | 'medium' | 'high';
  /**
   * MCP servers to provide access to additional external tools.
   *
   * Unified configuration that works for both LLM and computer agents:
   * - LLM agents: Configs are converted to MCPServer instances and provided as function tools
   * - Computer agents: Configs are passed directly to Codex SDK
   *
   * @example
   * ```typescript
   * mcpServers: [
  ...
  implements AgentConfiguration<TContext, TOutput>
{
  ...
  /**
   * Internal thread ID for session continuity.
   * For computer agents: Codex SDK thread ID
   * For LLM agents: Can be used for conversation continuity
   * @private
   */
  ...
  constructor(config: AgentOptions<TContext, TOutput>) {
    ...
    const providedModel = config.model;
    const providedModelName =
      typeof providedModel === 'string' ? providedModel : undefined;

    const derivedCodexType = parseCodexAgentType(providedModelName);
    let resolvedAgentType: AgentType;
    if (config.agentType) {
      resolvedAgentType = config.agentType;
    } else if (derivedCodexType) {
      resolvedAgentType = derivedCodexType;
    } else {
    ...
    if (
      config.agentType &&
      config.agentType !== 'llm' &&
      derivedCodexType &&
      derivedCodexType !== config.agentType
    ) {
      throw new UserError(
        `Provided model ${providedModelName} does not match agentType ${config.agentType}.`,
      );
    }

    if (resolvedAgentType !== 'llm') {
      if (providedModel !== undefined && typeof providedModel !== 'string') {
        throw new UserError(
          'Codex-backed agents require the model to be specified as a string.',
        );
      }
    }

    this.agentType = resolvedAgentType;

    // ========================================
    // Validate agent configuration
    // ========================================

    // COMPUTER AGENTS: Must have runtime, no tools (only MCP servers)
    if (resolvedAgentType === 'computer') {
      // Runtime is REQUIRED for computer agents
      if (!config.runtime) {
        throw new UserError(
          'Computer agents require a runtime. Please provide either LocalRuntime or CloudRuntime.\n' +
          'Example: new Agent({ agentType: "computer", runtime: new LocalRuntime(), ... })'
        );
      }

      // Tools are NOT supported (only MCP servers)
      if (config.tools && config.tools.length > 0) {
        throw new UserError(
          'Computer agents do not support function tools. ' +
          'Please use MCP servers instead via the mcpServers option.\n' +
          'Example: new Agent({ agentType: "computer", mcpServers: [...], ... })'
        );
      }
    }

    // LLM AGENTS: Must NOT have runtime
    if (resolvedAgentType === 'llm') {
      if (config.runtime) {
        throw new UserError(
          'LLM agents do not use runtime. Runtime is only for computer agents.\n' +
          'Please remove the runtime option for LLM agents.'
        );
      }
    }

    // Set model and tools based on agent type
    if (resolvedAgentType === 'computer') {
      // For computer agents, use provided model or empty placeholder (Codex SDK will use its default)
      // Note: Model must be a valid Claude model name (e.g., 'claude-sonnet-4'), not 'codex-computer'
      this.model = providedModelName ?? Agent.DEFAULT_MODEL_PLACEHOLDER;
    } else {
      this.model = providedModel ?? Agent.DEFAULT_MODEL_PLACEHOLDER;
    }
    ...
    this.workspace = config.workspace ? resolve(config.workspace) : process.cwd();
    this.reasoningEffort = config.reasoningEffort;

    // Initialize thread ID from sessionId (for backwards compatibility and explicit session resumption)
    this._threadId = config.sessionId;
  ...
  async getMcpTools(
    ...
    // Only LLM agents use MCP servers as tools
    // Computer agents pass MCP server configs to Codex SDK instead
    if (this.agentType === 'llm' && this.mcpServers.length > 0) {
      // Convert unified MCP configs to MCPServer instances for LLM agents
      const mcpServerInstances = mcpConfigsToServers(this.mcpServers);
  ...
  /**
   * Get the current session/thread ID for this agent.
   * Returns undefined if no session has been started yet.
   *
   * For computer agents: This is the Codex SDK thread ID that maintains conversation continuity.
   * For LLM agents: This can be used for conversation tracking.
   */
  ...
  /**
   * Internal method to update the thread ID after execution.
   * Called by the run loop to maintain session continuity.
   *
   * @internal
   * @param threadId - The new thread ID from execution
   */
  _updateThreadId(threadId: string): void {
    this._threadId = threadId;
  }

  /**
   * Reset the agent's session, starting fresh on the next run.
   * This clears the thread ID, so the next execution will create a new session.
   *
   * @example
   * ```typescript
   ...
   * await run(agent, 'Add tests'); // Continues same session
   *
   * agent.resetSession();
   ...
   * This allows you to continue a previous conversation/session.
   *
   * @param sessionId - The session/thread ID to resume
   *
   * @example
   * ```typescript
   * const agent = new Agent({ agentType: 'computer', runtime });
   ...
   * agent.resumeSession('thread-abc-123');
   * await run(agent, 'Continue previous work');
   * ```
   */
