...
export type RunConfig = {
   ...
   * Not required for computer agents which bypass the model provider entirely.
   */
...
function agentInputToText(input: string | AgentInputItem[]): string {
...
export class Runner extends RunHooks<any, AgentOutputType<unknown>> {
  ...
  constructor(config: Partial<RunConfig> = {}) {
    super();
    this.config = {
      modelProvider: config.modelProvider, // Lazy load when needed
      model: config.model,
  ...
  /**
   * Get the model provider, lazily loading the default if not set.
   *
   * @internal
   */
  ...
  /**
   * Execute a computer-use agent via Runtime (LocalRuntime or CloudRuntime).
   *
   * Computer agents bypass the standard model provider flow entirely and execute
   * directly through Codex CLI. This provides a simpler execution path optimized
   * for computer-use tasks.
   *
   * @internal
   */
  async #executeComputerAgent<
    TContext,
  ...
  >(
    ...
    const workspace = state._currentAgent.workspace;
    const repoPath = isAbsolute(workspace)
      ? workspace
      : resolvePath(workspace);

    // Convert input to text (computer agents receive tasks as text)
    const taskText = agentInputToText(state._originalInput);

    // Runtime is required for computer agents (validated in Agent constructor)
    const runtime = state._currentAgent.runtime!;

    // Get current thread ID from agent (for session continuity)
    const currentThreadId = state._currentAgent.currentThreadId;

    // Execute via runtime (local or cloud)
    const result = await runtime.execute({
      agentType: 'computer',
      task: taskText,
      workspace: repoPath,
      sessionId: currentThreadId,
      model: typeof state._currentAgent.model === 'string' && state._currentAgent.model
        ? state._currentAgent.model
        : undefined,
      reasoningEffort: state._currentAgent.reasoningEffort,
      mcpServers: state._currentAgent.mcpServers, // Unified type - no assertion needed!
    });

    // Update thread ID on agent for continuity across multiple runs
    // This ensures the next run() call continues the same conversation
    if (result.sessionId) {
      state._currentAgent._updateThreadId(result.sessionId);
    }
  ...
  >(
    ...
    return withNewSpanContext(async () => {
      ...
      // ============================================================
      // COMPUTER AGENT: Direct execution path (bypasses model provider)
      // ============================================================
      // Computer agents execute via Runtime (LocalRuntime or CloudRuntime)
      // using Codex CLI. They do NOT use the standard model provider flow.
      // This provides a simpler, more direct execution path for computer-use tasks.
      if (
        !isResumedState &&
        state._currentAgent.agentType === 'computer'
      ) {
        return this.#executeComputerAgent(state);
      }

      // ============================================================
      // LLM AGENT: Standard model provider flow
      // ============================================================
      ...
      try {
        while (true) {
          ...
          if (typeof model === 'string') {
            model = await this.#getModelProvider().getModel(model);
          }
